        ...
        private void InitVertices()
        {
            ...
            for(float x = start.x; x < end.x; x += distanceBetweenVertices) {
                for(float y = start.y; y < end.y; y += distanceBetweenVertices) {
                    ...
                    var walls = Physics2D.OverlapCircleAll(vertex.position, minDistanceFromWalls, 0);
                    if(walls.Length == 0) {
                        vertices.Add(vertex);
                    }
                }
            ...
            foreach(var vertex in vertices) {
                ...
                foreach(var other in vertices) {
                    ...
                    if(distance < distanceBetweenVertices + 0.2f) {
                        ...
                        var hit = Physics2D.Raycast(vertex.position, (other.position - vertex.position).normalized, distanceBetweenVertices, 0);
